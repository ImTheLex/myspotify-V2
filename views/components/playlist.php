...
<article class="p-rel td-5 -scale-1 mb-2 body-grad-3 playlist-left-container">
    <div class="center-b gap-x-3 px-2 w-full rounded-s pr-2 py-2 hovr-cus-7 hovr-bg-cus-8 br-1-s br-cus-c-7">
        <a href='<?= isset($playlistitem) && $playlistitem ? "/controllers/PlaylistController.php?bOpenPlaylist={$playlistitem['id']}" : ''?>' class="items-center gap-x-3 w-3/5">
        ...
        <form class="w-12 h-12 center c-p mr-4" action="/controllers/PlaylistController.php" method="post">
            <?php if ($playlistitem['user_id'] == $userdatas['id']):?>
                ...
                <button type="submit" class="c-p bg-trans br-none appearance-none" aria-label="supprimer playlist" name="bDropPlaylist">
                    <svg class=" hovr-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
